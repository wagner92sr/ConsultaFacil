class Site::Profile::MyDataController < Site::ProfileController
  ...
  def update
    if @profile_member.update(params_profile_member)
      redirect_to edit_site_profile_my_datum_path(current_paciente.id), notice: "Perfil atualizado com sucesso!"
    else
      render :edit
    end
  end
  ...
  private
    ...
    def set_profile_member
      @profile_member = ProfileMember.find_or_create_by(paciente_id: current_paciente.id)
    end
